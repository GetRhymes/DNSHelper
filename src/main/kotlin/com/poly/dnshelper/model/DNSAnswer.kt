...
data class DNSAnswer(
    ...
    fun mapperAnswer(byteArray: ByteArray, domainName: String) {
        ...
        timeToLive = getIntFromBytes(byteArray.toList().subList(domainName.length + 5, domainName.length + 9).toByteArray())
        dataLength = getShortFromTwoBytes(byteArray[domainName.length + 9] to byteArray[domainName.length + 10])
        resourceData = String(byteArray.toList().subList(domainName.length + 11, domainName.length + 11 + dataLength).toByteArray())
    }
}
